...
> Q. How many dimensions are in a qubit’s vector space? > A. Two.
...
> Q. Once activated, a service worker **???** , then transitions to idle. From > that state, it’ll handle fetch or message events until it eventually > terminates. > A. Once activated, a service worker _performs one-time startup computation_ > , then transitions to idle. From that state, it’ll handle fetch or message > events until it eventually terminates.
