...
> Q. What function is its own derivative? > A. `e` > Q. What is the derivative of `f(x) = e^x`? > A. `e^x`

But we could also imagine framing this as an application prompt.

> Q. What’s the derivative of `e^8x`? > A. `8e^8x` [Explanation: the derivative of e^x is itself]()
...
> Q. Let `f(x) = 14e^[x]`. What is `df/dx`? > A. `14e^[x]` [Explanation: the derivative of e^x is itself]()
